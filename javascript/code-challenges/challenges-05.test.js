...
const addPurchases = (arr) => {
  if (arr.length === 0)
    return 0;
  return arr.reduce((sum, item) => sum.purchasePrice + item.purchasePrice);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((sum, val) => sum + 1);
};
...
const returnNames = (arr) => {
  let namesArr = [];
  arr.forEach((item, i) => namesArr.push(item.name));
  return namesArr;
};
